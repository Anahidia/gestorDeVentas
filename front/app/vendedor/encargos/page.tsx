...
export default function MisEncargosPage() {
  ...
  const handleComplete = async (id: string) => {
    try {
      ...
      alert(error.message || "Error al completar el encargo")
    }
  }
  ...
  const handleCancel = async (id: string) => {
    if (confirm("¿Estás seguro de cancelar este encargo?")) {
      try {
        ...
        alert(error.message || "Error al cancelar el encargo")
      }
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-3xl font-bold text-foreground">Mis Encargos</h1>
        <p className="text-muted-foreground">Gestiona tus reservas de productos</p>
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        {orders.length === 0 ? (
          <p className="text-muted-foreground">No tienes encargos</p>
        ) : (
          orders.map((order) => (
